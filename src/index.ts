const src = document.getElementById("source") as HTMLCanvasElement;
if (!src) throw "Source Not Found";
const ctx: CanvasRenderingContext2D | null = src.getContext("2d");
if (!ctx) throw "Source Canvas Context Not Found";
...
class Ball {
  ...
  constructor(radius: number = 50, color: string = "rgb(255,0,0)") {
    this.color = color;
    this.radius = radius;
  ...
  render(ctx: CanvasRenderingContext2D, dt: number, t: number) {
    ctx.save();
    ctx.beginPath();
  ...
  tick(ctx: CanvasRenderingContext2D, dt: number, t: number) {
    dt = dt / 1000;
  ...
  checkBounds(ctx: CanvasRenderingContext2D) {
  ...
  checkRight(ctx: CanvasRenderingContext2D, paddle: Paddle) {
  ...
  checkLeft(ctx: CanvasRenderingContext2D, paddle: Paddle) {
    const xLowerBound = paddle.x - paddle.width / 2 + this.radius;
    const xUpperBound = paddle.x + paddle.width / 2 + this.radius;
...
class Paddle {
  ...
  constructor(width: number = 20, height: number = defaultPaddleHeight, speed: number = 750, color: string = "rgb(255,255,255)") {
    this.x = 0;
    this.y = 0;
...
// Calculation Step
function calculate(ctx: CanvasRenderingContext2D, dt: number, t: number) {
  if (paused) return;
...
function drawCountDown(ctx: CanvasRenderingContext2D, number: number) {
  ctx.save();
  ctx.font = "50px Arial";
...
function render(ctx: CanvasRenderingContext2D, dt: number, t: number) {
  if (!ctx) return;
  clearCanvas(ctx);
...
// Called Every Frame
var previousTime: number = 0; // milliseconds
...
// Initialize Game
const playerA: Paddle = new Paddle(); // left
const playerB: Paddle = new Paddle(); // right

const ball: Ball = new Ball();

const inset: number = 50;
...
// Options
const gravity_mode = document.getElementById("gravity_mode") as HTMLInputElement;
if (gravity_mode) {
  gravity_mode.addEventListener("input", (event: Event) => {
    if (gravity_mode.checked) {
      ball.accY = gravity;
    } else {
      ball.accY = 0;
...
const short_paddle = document.getElementById("short_paddle") as HTMLInputElement;
if (short_paddle) {
  short_paddle.addEventListener("input", (event: Event) => {
    if (short_paddle.checked) {
      playerA.height = shortPaddleHeight;
      playerB.height = shortPaddleHeight;
    } else {
...
const long_paddle = document.getElementById("long_paddle") as HTMLInputElement;
if (long_paddle) {
  long_paddle.addEventListener("input", (event: Event) => {
    if (long_paddle.checked) {
      playerA.height = longPaddleHeight;
      playerB.height = longPaddleHeight;
    } else {
